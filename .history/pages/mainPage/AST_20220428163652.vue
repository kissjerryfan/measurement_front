            ...
            this.getLoc_(formData).then((res)=>{
                ...
                this.getAST_(formData).then((res)=>{
                    ...
                    for(let i = 0; i<res.length;i++){
                        let temp = {'key': i, 'call':'', 'reference':'', 'name': res.key}
                        let s = ''
                        if(res.raw.call != []){
                            console.log('aaxx', res.raw.call)
                            for(let j = 0;j<res.raw.call.length;j++) s += res.raw.call[j] + ', '
                            temp['call'] = s.substring(0, s.length-2)
                        }
                        s = ''
                        if(res.raw.reference != []){
                            for(let j = 0;j<res.raw.reference.length;j++) s += res.raw.reference[j] + ', '
