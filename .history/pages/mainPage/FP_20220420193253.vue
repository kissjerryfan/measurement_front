<template>
    <div>
        ...
        <div class="FP">
          <h2>VAF计算</h2>
            <a-table :columns="fp_vaf_columns" :data-source="fp_vaf_data" :pagination="false" bordered>
                <template v-for="col in ['id', 'feature', 'description']" :slot="col" slot-scope="text, record, index">
                    <div :key="col">
                        <template>
                            {{ text }}
                        </template>
                ...
                <template v-for="col in ['level']" :slot="col" slot-scope="text, record, index">
                    <div :key="col">
                        <a-input v-if="record.editable" style="margin: -5px 0" :value="text"
        ...
        <div class="FP">
          <h3 style="margin-top:8px">该项目功能点度量结果FP为{{fp_vaf * fp_ufc}}</h3>
        </div>
    </div>
</template>
<script>
