...
export default {
    ...
    methods:{
        ...
        handleUpload() {
            ...
            this.getAST_(formData).then((ress)=>{
                    this.dataList = []
                    this.LCOM = ress.lcom
                    this.RFC = ress.rfc
                    this.className = ress.classname
                    let ct = 0
                    console.log('aa', ress.raw)
                    for(let i in ress.raw){
                        console.log('aaxx', i)
                        let temp = {'key': ct, 'call':'', 'reference':'', 'name': i}
                        let s = ''
                        if(i.call != []){
                            for(let j = 0;j<i.call.length;j++) s += i.call[j] + ', '
                            temp['call'] = s.substring(0, s.length-2)
                        }
                        s = ''
                        if(i.reference != []){
                            for(let j = 0;j<i.reference.length;j++) s += i.reference[j] + ', '
                            temp['reference'] = s.substring(0, s.length-2)
                        }
                        this.dataList.push(temp)
                        ct++;
                    }
                    console.log('ast',ress)
                },(error)=>{
