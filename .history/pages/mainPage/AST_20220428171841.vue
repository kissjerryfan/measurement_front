<template>
    <div>
        ...
        <div class="bd">
            <h2>抽象语法树分析结果</h2>
            <a-table :columns="columns" :data-source="dataList" class="components-table-demo-nested" slot-scope="record">
                <a-table 
        ...
        handleUpload() {
            ...
            this.getAST_(formData).then((res)=>{
                ...
                for(let i = 0;i<res.length;i++){
                    ...
                    let k = 0
                    for(let mtd in res.raw){
                        let md = {'key': k, 'method': mtd, 'call': raw[mtd].call, 'reference': raw[mtd].reference}
                        temp['innerData'].push(md)
                    }
                    this.dataList.push(temp)
                }
                console.log('ast',this.dataList)
            },(error)=>{
